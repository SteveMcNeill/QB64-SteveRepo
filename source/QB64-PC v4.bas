...
REDIM UserDefine(1, 100) AS STRING '0 element is the name, 1 element is the string value
REDIM SHARED InValidLine(10000) AS _BIT
DIM DefineElse(255) AS _BYTE
DIM UserDefineCount AS INTEGER
...
DO
    ...
    IF LEN(wholeline$) THEN
        ...
        IF LEFT$(temp$, 4) = "$IF " THEN
            ...
            IF temp = 0 THEN
                'no = sign in the $IF statement, so we're going to assume the user is doing something like $IF flag
                FOR i = 0 TO UserDefineCount
                    IF UserDefine(0, i) = temp$ THEN 'We found the flag
                        IF UserDefine(1, i) <> "0" THEN 'And it wasn't set to 0
            ...
            l$ = RTRIM$(LEFT$(temp$, temp - 1)): r$ = LTRIM$(MID$(temp$, temp + 1))
            FOR i = 0 TO UserDefineCount
                IF UserDefine(0, i) = l$ THEN 'we found the flag
                    IF UserDefine(1, i) = r$ THEN 'we checked to see if the values matched
                        ExecLevel(ExecCounter) = ExecLevel(ExecCounter - 1) 'if they do then we inherit the permissions from an above level
                        IF ExecLevel(ExecCounter) = 0 THEN DefineElse(ExecCounter) = DefineElse(ExecCounter) OR 4 'Else if used and conditon found
                    END IF
                    GOTO finishedlinepp 'and either way we're finished
                END IF
            NEXT 'other wise we keep looking until we go through the whole list
...
DO
    ...
    IF InValidLine(linenumber) THEN layoutdone = 0: GOTO finishednonexec
